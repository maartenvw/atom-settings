...
void GitSubmodule::Foreach_callback_async(uv_async_t* req, int status) {
  ...
  Local<Value> argv[3] = {
          GitSubmodule::New((void *)baton->sm, false),
...
void GitSubmodule::Foreach_callback_setupAsyncPromisePolling(uv_async_t* req) {
  Foreach_CallbackBaton* baton = static_cast<Foreach_CallbackBaton*>(req->data);
  uv_async_init(uv_default_loop(), &baton->req, (uv_async_cb) Foreach_callback_asyncPromisePolling);
  uv_async_send(&baton->req);
}

void GitSubmodule::Foreach_callback_asyncPromisePolling(uv_async_t* req, int status) {
  Nan::HandleScope scope;
